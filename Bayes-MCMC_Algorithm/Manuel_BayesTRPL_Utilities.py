...
def unpack_Data(FileName):
    ...
    return time2, Data3, len_Data, Bckg2
...
def unpack_filenames(FileNames, intensity, Reflectance, laserpower_file):
    ...
    for FileName in FileNames:
        ...
        time2, Data4, len_Data1, Bckg2 = unpack_Data(FileName)
        data.append(Data4)
        time.append(time2)
        len_Data.append(len_Data1)
        bckg_list.append(Bckg2)
    ...
    return df, pile_up, sample_names, Fluence, bckg_list
...
def Bayes_TRPL_Utils(data_folder_trpl, config_file_name, laserpower_file):
    ...
    df, pile_up, sample_name, Fluence, bckg_list = unpack_filenames(FileNames, intensity, Reflectance, laserpower_file)

    amax = np.where(np.array(df['Time']) <= max_time )[0][-1]

    print('Files ready...')
    return df, pile_up, sample_name, Fluence, Thickness, Surface, Absorption_coeff, Reflectance, intensity, amax, bckg_list
